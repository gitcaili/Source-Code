...
{
    public class PlanSendToSelfPage : Web
    {
        ...
        {
            // AddContent("NOT DOOMED");   
            orgFilePath = GetLibraryPath() + @"\Plan\QuoteSendToSelf\Template\SendToSelf.xls";

            string checkPathGenTemplate = GetLibraryPath() + @"\Plan\QuoteSendToSelf\GeneratedTemplate";
            ...
            else
            {
                ...
                try
                {
                    ...
                    //mWorkBook = oXL.Workbooks.Open(orgFilePath);
                    ...
                    //'Set Agency Name   
                    //  AddContent("Send to self Head");                 
                    SetAgency();
                    //set detail grid value
                    //AddContent("Send to self details");
                    SetDetailGrid();
                }
                catch (Exception ex)
                {
                ...
                //'S.Y. 22May15
                string date = "";
                string filePath = GetLibraryPath();
                date = DateTime.Now.Date.ToShortDateString().Replace(@"/", "-");

                string basePath = @"Plan\QuoteSendToSelf\GeneratedTemplate\SendToSelf_" + max + ".xls";
                //mergedFilePath = GetLibraryPath() + @"\Plan\QuoteSendToSelf\GeneratedTemplate\SendToSelf_" + date + DateTime.Now.Millisecond.ToString() + ".xls";

                mergedFilePath = filePath + basePath;
                ...
                catch (Exception ex)
                {
                    ...
                    AddContent(ex.Message.ToString());
                    //  AddContent(System.Runtime.InteropServices.Marshal.ReleaseComObject(oXL).ToString());
        ...
        private void SetAgency()
        {
            ...
            //  AddContent(sQuoteVersionRef + "DUCK" + sNewsWorksRef);
        ...
        private void SetDetailGrid()
        {
            ...
            Range RngToCopy = range2.EntireRow;
            QuerySelect objPblcnRec = GetQuery();
            objPblcnRec.SQLCommand = strSQL;
            objPblcnRec.ExecuteReader();
            ...
            while (!objPblcnRec.Eof() && !string.IsNullOrEmpty(objPblcnRec.FieldValue("pblc_Name")))
            {
                //   AddContent("FIGHT");
                ...
                if (!string.IsNullOrEmpty(objPblcnRec.FieldValue("pnbr_commissiontype").ToString()))
                {
                    ...
                    else { mWSheet1.Cells[j, 64] = "Non Commission"; }
                }
                ...
                if (!string.IsNullOrEmpty(objPblcnRec.FieldValue("pnbr_date").ToString()))
                {
                    ...
                    // mWSheet1.Cells[j, 19] = (objPblcnRec.FieldValue("pnbr_date").ToString().Substring(0, 9));
                }
                if (!string.IsNullOrEmpty(objPblcnRec.FieldValue("pblc_Name").ToString()))
                {
                    // mWSheet1.Cells[j, 5] = objPblcnRec.FieldValue("pblc_Name").ToString();
                }
                if (!string.IsNullOrEmpty(objPblcnRec.FieldValue("pnbr_standardsize").ToString()))
                {
                ...
                if (!string.IsNullOrEmpty(objPblcnRec.FieldValue("pnbr_ratecard").ToString()))
                {
                    ...
                    if (!objRec.Eof())
                    {
                        ...
                        int widthlim = -1;
                        if (objPblcnRec.FieldValue("sctn_name").ToString() != "Inserts")
                        {
                ...
                //string type = objPblcnRec.FieldValue("pnbr_commissiontype").ToString();
                if (type != "NonCommission")
            ...
            mWSheet1.Cells[end - 1, 68] = bookingtotal.ToString("C2");
...

        {
            ...
            AddContent(objPnbrDay + "GFIBGDSF");
            if (arrDay.Length > 1)
            {
                while (true && arrDay.Length > 1)
                {
                    // AddContent(arrDay[1] + "TAKE MY MONEY");
                    switch (arrDay[1])
                    {
                        case "Mon":
                            sDay += "Monday,";
