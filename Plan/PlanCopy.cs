            ...
            try
            {
                ...
                if (strEntityId > 0)
                {
                    ...
                    */
                    ...
                    if (!String.IsNullOrEmpty(Dispatch.EitherField("CheckedPlanId")))
                    {
                        sPlanIdString = Dispatch.EitherField("CheckedPlanId");
                        string[] Plans = sPlanIdString.Split(',');
                        foreach (string PlanID in Plans)
                        {
                            //AddContent("<BR>PlanID=" + PlanID);
                            Record RecEachPlanBuilder = FindRecord(strEntityNameSeconday, "pnbr_Pnbr_planbuilderid=" + PlanID);

                            if (!RecEachPlanBuilder.Eof())
                            {
                                //AddContent("<BR>PlanID 1=" + PlanID);
                                Record objPlanBuilder = new Record(strEntityNameSeconday);
                                objPlanBuilder.SetField("pnbr_plan", strEntityId);
