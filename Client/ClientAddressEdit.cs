...
namespace NZPACRM
{
    public class ClientAddressEdit : Web
    {
        ...
        int iAddressId = 0;
        #endregion
        public ClientAddressEdit()
        {
            GetTabs("Client");
            AddTopContent(GetCustomEntityTopFrame("Client"));
            #region Set Client ID
            ...
            AddUrlButton("Cancel", "Cancel.gif", UrlDotNet(this.ThisDotNetDll, "RunClientAddressList"));
            #endregion
        ...
        public override void BuildContents()
        {
            try
            {
                string sAddressSQL = " select * from vClientAddress where client_ClientID=" + sClientId + " and adli_Addressid=" + sAddressId + " ";
                ...
                if (!sQueryObj.Eof())
                {
                    ...
                    if (!string.IsNullOrEmpty(sQueryObj.FieldValue("client_PrimaryClientId")))
                        sDefaultAddress = sQueryObj.FieldValue("client_PrimaryClientId");
                    else
                        sDefaultAddress = "";
                    ...
                    if (sDefaultAddress != "0" && sDefaultAddress != "")
                    {
                        if(sDefaultAddress == sAddressId)
                            sHTML += HTML.TableData("Set as default address for Client  " + HTML.InputCheckBox("addr_default", true), "VIEWBOXCAPTION");
                        else
                            sHTML += HTML.TableData("Set as default address for Client  " + HTML.InputCheckBox("addr_default", false), "VIEWBOXCAPTION");
                    }
                ...
                if (sHidden == "Save")
                {
                    ...
                    string sURL = UrlDotNet(this.ThisDotNetDll, "RunClientAddressList");
                ...
                if (sHiddenDelete == "Delete")
                {
                    ...
                    string sURL = UrlDotNet(this.ThisDotNetDll, "RunClientConfirmDelete&client_ClientID=" + sClientId + "&Addr_AddressId="+sAddressId);
                    Dispatch.Redirect(sURL);
                } 
            }
        ...
        {
            Record objAddressLinkRec = FindRecord("address_link", "adli_addressid=" + sAddressId + " and adli_clientid=" + sClientId + " and LTRIM(RTRIM(adli_type))='" + sType.Trim() + "' ");
            if (!objAddressLinkRec.Eof())
            {
                ...
                objAddressLinkRec.SetField("adli_clientid", sClientId);
        ...
        public void CreateNewAddressLink(string AddressId, string sFrom)
        {
            ...
            objAddressLinkRec.SetField("AdLi_CompanyID", sCompanyid);
            objAddressLinkRec.SetField("AdLi_PersonID", sPersonId);
            objAddressLinkRec.SetField("adli_addressid", sClientId);
        ...
        {
            Record objBuildingRec = FindRecord("client", "client_ClientID=" + sClientId + " and client_Deleted is null");
            if (!objBuildingRec.Eof())
            {
                objBuildingRec.SetField("client_PrimaryClientId", sAddressId);
        ...
        public void DeleteDefaultAddress()
        {
            ...
            if (!objBuildingRec.Eof())
            {
                objBuildingRec.SetField("client_PrimaryClientId", "");
                objBuildingRec.SaveChanges();
            }
        }
    }
