...
namespace NZPACRM
{
    public class ClientPhoneEmail : Web
    {
        CRMHelper objCRMHelper = new CRMHelper();
        public ClientPhoneEmail()
        {

            //AddContent("inside");
            //'Set Tabs
            objCRMHelper.SetTabs("Client");

            //'Set Context of Custom Entity
            objCRMHelper.SetCustomEntityTopFrame("Client");
        }
        ...
        public override void BuildContents()
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(Dispatch.EitherField("client_ClientID")))
                    iEntityRecordID = Dispatch.EitherField("client_ClientID");
                else
                ...
                if (!objEmailRec.Eof())
                {
                    ...
                    while (!objEmailRec.Eof())
                    {
                        ...
                        if (!String.IsNullOrEmpty(sEmailTypeCaption))
                        {
                            ...
                            objEmailSQL += " where elink_Deleted is null and emai_deleted is null and ELink_EntityID != 0  and elink_recordid=" + iEntityRecordID;
                            QuerySelect sQueryObj = GetQuery();
                ...
                sHTMLDetails += HTML.TableRow("", "CONTENT").ToString();
                sHTMLDetails += HTML.TableData(HTML.Box("Phone", sHTMLPhoneDetails), "SIMPLEGRIDBOLD");
                sHTMLDetails += HTML.TableData("", "TABLEHEADBLANK");
                sHTMLDetails += HTML.TableData(HTML.Box("Email", sHTMLEmailDetails), "SIMPLEGRIDBOLD");
                sHTMLDetails += HTML.EndTable();
                //sHTMLDetails += HTML.TableData("<Table Width='30%'> <TR> <TD> </TD></TR></Table>");                
                ...
                if (!String.IsNullOrEmpty(Dispatch.ContentField("HIDDENPAGENUMBER_Save")))
                {
                    ...
                    {                        
                        int iEntityId = 0;
                        Record objEntityIdRec = FindRecord("custom_tables", "bord_name='Client' and bord_deleted is null ");
                        ...
                        int count = objPhoneTranslations.RecordCount;
                        ...
                        if (!objPhoneTranslations.Eof())
                        {
                            while (!objPhoneTranslations.Eof())
                            {
                                ...
                                AddContent("sPhoneTypeTranslation" + sPhoneTypeTranslation);
                                AddContent(Dispatch.ContentField("phon_countrycode" + sPhoneTypeTranslation.ToLower().TrimEnd(':')));
                                ...
                                if (!String.IsNullOrEmpty(Dispatch.ContentField("phon_countrycode" + sPhoneTypeTranslation.ToLower().TrimEnd(':'))))
                                {
                                    AddContent("Phone Data");
                ...
                else
                {
                    ...
                    #region Add Buttons
                    string sUrl = "javascript:document.EntryForm.HIDDENPAGENUMBER_Save.value='Save';";
