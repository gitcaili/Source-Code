...
namespace NZPACRM
{
    public class ClientAddress : Web
    {
        ...
        public override void BuildContents()
        {
            ...
            {                
                #region Get Address
                objCRMHelper.AddressBox("Client");
                #endregion            
                #region Get Client id                 
                    if (!String.IsNullOrEmpty(Dispatch.EitherField("client_ClientID")))
                        ClientID = Dispatch.EitherField("client_ClientID");
                    else
                        ClientID = Dispatch.EitherField("Key58");
                #endregion
        ...
        {            
            ...
            string[] objParaName = new string[] { "AdLi_CompanyID", "AdLi_PersonID", "AdLi_AddressId", "adli_type", "adli_clientid" };
            string[] objParaValue = new string[] { sCompanyid, sPersonId, AddressId.ToString(), sFrom, ClientID };
            objCRMHelper.CreateNewAddress(objEntity, objParaName, objParaValue);            
            if (sDefaultAddress != "")
            {
                Record objClientRec = FindRecord("Client", "client_ClientID=" + ClientID);
                if (!objClientRec.Eof())
                {
                    objClientRec.SetField("client_primaryclientid", AddressId.ToString());
