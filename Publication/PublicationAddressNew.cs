...
namespace NZPACRM
{
    public class PublicationAddressNew : Web
    {
        ...
        public override void BuildContents()
        {
            try
            {
                ...
                #endregion
                #region Get Address
                objCRMHelper.AddressBox("Publications");
                #endregion
        ...
        public void CreateNewAddressLink(string sCompanyid, string sPersonId, int AddressId, string sFrom)
        {
            ...
            string[] objParaName = new string[] { "AdLi_CompanyID", "AdLi_PersonID", "AdLi_AddressId", "adli_type", "adli_publicationid" };
            string[] objParaValue = new string[] { sCompanyid, sPersonId, AddressId.ToString(), sFrom, sPublicationId };
            objCRMHelper.CreateNewAddress(objEntity, objParaName, objParaValue);
            if (sDefaultAddress != "")
            {                
                Record objClientRec = FindRecord("Publications", "pblc_PublicationsID=" + sPublicationId);
                if (!objClientRec.Eof())
                {
                    objClientRec.SetField("pblc_primarypublicationid", AddressId.ToString());
