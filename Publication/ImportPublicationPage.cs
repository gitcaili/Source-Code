...
namespace NZPACRM
{
    public class ImportPublicationsPage : DataPageNew
    {
        CRMHelper objCRM = new CRMHelper();
        string LogfileName = "";
        public ImportPublicationsPage()
        ...
        public override void BuildContents()
        {
            try
            {
                ...
                objCRM.GetTemplateBlock("Publications");
                #endregion
                ...
                if (!String.IsNullOrEmpty(Dispatch.ContentField("HIDDEN_Save")))
                {
                    if (Dispatch.ContentField("HIDDEN_Save") == "Save")
                    {
                        ...
                        {
                            #region Save File in Publication folder
                            SavedFilePath = SaveFileAtPublicationLocation();
                            #endregion
                            ...
                            #region Find EnityID
                            Record RecEnityID = FindRecord("Custom_Tables", "bord_name='Publications' and bord_deleted is null");
                            if (!RecEnityID.Eof())
                            {
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                if (dt.Rows[0][0].ToString().Trim().ToLower() == "publication" || dt.Rows[0]["Type"].ToString().Trim().ToLower() == "publication")
                                {
                                    for (int i = 0; i < dt.Rows.Count; i++)
                                    {
                                        ...
                                        string PersonID = "";
                                        string Website = "";
                                        #region Inserting Publication
                                        if (dt.Rows[i][0].ToString().Trim().ToLower() == "publication" || dt.Rows[i]["Type"].ToString().Trim().ToLower() == "publication")
                                        {
                                            ...
                                            if (sWhereClause != "")
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
                                                        Generatelog("[" + System.DateTime.Now.ToString() + "]" + PersonFName + " " + PersonLName + " No Person is attached in CRM");
                                                    }
                                                }
                                            }
                                            #endregion
                                            ...
                                            Record recFindPublication = FindRecord("Publications", "pblc_Name='" + Name.Replace("'","''") + "'");
                                            if (recFindPublication.RecordCount == 0)
                                            {
                                               
                                                #region Insert New Publication record
                                                string[] ParaName = new string[] { "pblc_Name", "pblc_PublishersID", "pblc_RatesCardID", "pblc_UserId", "pblc_website", "pblc_CreatedBy", "pblc_CreatedDate" };
                                                string[] ParaValue = new string[] { Name, "", "", CurrUser, Website, System.DateTime.Now.ToString() };
                                                PbNewRecordID = objCRM.InsertUpdateEnity("publications", ParaName, ParaValue);
                                                #endregion
                                            }
                                            else
                                            {
                                                #region Update Publication record
                                                PbNewRecordID = Convert.ToInt32(recFindPublication.GetFieldAsString("pblc_PublicationsID"));
                                                if (!recFindPublication.Eof())
                                                {
                                                    recFindPublication.SetField("pblc_website", Website);                                                    
                                                    recFindPublication.SaveChanges();
                                                }
                                                #endregion
                                            }
                                        }
                                        ...
                                        if (PbNewRecordID != 0)
                                        {
                                            ...
                                            if (Address1 != "" || Address2 != "" || Address3 != "" || Address4 != "" || PhysicalCity != "" || PhysicalZipCode != "" || PhysicalCountry != "")
                                            {
                                                ...
                                                Record objPhysicalAddressRec = FindRecord("Address", " Addr_address1= '" + Address1 + "' and addr_type ='Physical' and  Addr_AddressId in (select adli_addressid from Address_Link where AdLi_Type is null and adli_publicationid ='" + PbNewRecordID + "' and adli_deleted is null)");
                                                if (!objPhysicalAddressRec.Eof())
                                                {
                                                    //update address
                                                    Record objUpdateAddressRec = FindRecord("Address", "addr_address1='" + Address1 + "' and addr_type ='Physical' and  Addr_AddressId in (select adli_addressid from Address_Link where AdLi_Type is null and adli_publicationid ='" + PbNewRecordID + "' and adli_deleted is null)");
                                                    ...
                                                    if (PhysicalType == "Shipping" || PhysicalType == "Business" || PhysicalType == "Billing")
                                                    {
                                                        ...
                                                        else
                                                        {
                                                            ...
                                                            objaddresslink.SetField("adli_publicationid", PbNewRecordID);
                                                ...
                                                else
                                                {
                                                    ...
                                                    Record objClientRec = FindRecord("Publications", "pblc_PublicationsID=" + PbNewRecordID);
                                                    if (!objClientRec.Eof())
                                                    {
                                                    ...
                                                    objaddresslink1.SetField("adli_publicationid", PbNewRecordID);
                                            ...
                                            if (PostalAddr1 != "" || PostalAddr2 != "" || PostalAddr3 != "" || PostalAddr4 != "" || PostalCity != "" || PostalCountry != "" || PostalZipCode != "")
                                            {
                                                ...
                                                Record objPhysicalAddressRec = FindRecord("Address", " Addr_address1= '" + PostalAddr1 + "' and addr_type ='Postal' and  Addr_AddressId in (select adli_addressid from Address_Link where AdLi_Type is null and adli_publicationid ='" + PbNewRecordID + "' and adli_deleted is null)");
                                                if (!objPhysicalAddressRec.Eof())
                                                {
                                                    //update address

                                                    Record objUpdateAddressRec = FindRecord("Address", "addr_address1='" + PostalAddr1 + "' and addr_type ='Postal' and  Addr_AddressId in (select adli_addressid from Address_Link where AdLi_Type is null and adli_publicationid ='" + PbNewRecordID + "' and adli_deleted is null)");
                                                    if (!objUpdateAddressRec.Eof())
                                                    {
                                        ...
                                        ImportCount++;
                                        Generatelog("[" + System.DateTime.Now.ToString() + "]" + " Publication " + Name + " is imported sucessfully in Sage CRM.");
                                    }

                                    string sURL = UrlDotNet(this.ThisDotNetDll, "RunPublicationImportCompletePage") + "&imported=" + ImportCount + "&LogFileName=" + LogfileName;
                                    Dispatch.Redirect(sURL);
                                }
                                else
                                {
                                    string sFailURL = UrlDotNet(this.ThisDotNetDll, "RunPublicationImportCompletePage") + "&Fail=Fail";
        ...
        public void Generatelog(string Logcontent)
        {
            ...
            try
            {
                ...
                //string ymd = System.DateTime.Now.Hour.ToString() + System.DateTime.Now.Minute.ToString() + System.DateTime.Now.Second.ToString() + theDate.ToString("yyyyMMdd") + "PublicationsLog.txt";
                string ymd = System.DateTime.Now.Hour.ToString() + System.DateTime.Now.Minute.ToString() + theDate.ToString("yyyyMMdd") + "PublicationsLog.txt";
                LogfileName = ymd;
        ...
        private string SaveFileAtPublicationLocation()
        {
            string FileName = "";
            string newFullPath = "";
            ...
            NewPath += "WWWRoot\\CustomPages\\NZPAImport\\ImportedFiles\\Publications\\";
