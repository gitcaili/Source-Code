...
namespace NZPACRM
{
    public class PublisherAddressEdit : Web
    {
        ...
        int iAddressId = 0;
        #endregion
        public PublisherAddressEdit()
        {
            #region Set Tab and Top content
            //'Set Tabs
            //objCRMHelper.SetTabs("Publishers");
            GetTabs("Publishers", "Phone / Email");
        ...
        public override void BuildContents()
        {
            try
            {
                ...
                string sAddressSQL = "select * from  vPublisherAddressList where pbls_PublishersID = " + sPublishrId + " and adli_Addressid =" + sAddressId;
                ...
                if (!sQueryObj.Eof())
                {
                    ...
                    if (!string.IsNullOrEmpty(sQueryObj.FieldValue("pbls_primarypublisherid")))
                        sDefaultAddress = sQueryObj.FieldValue("pbls_primarypublisherid");
                    else
                        sDefaultAddress = "";
                    ...
                    sHTML += HTML.TableRow("");
                    sHTML += HTML.TableData(HTML.InputText("addr_address1", sAddress1, 40, 20, "", "", false, "tabindex=1"));
                    ...
                    {                        
                        if (sDefaultAddress == sAddressId)
                            sHTML += HTML.TableData("Set as default address for Publishers  " + HTML.InputCheckBox("addr_default", true), "VIEWBOXCAPTION");
                        else
                            sHTML += HTML.TableData("Set as default address for Publishers  " + HTML.InputCheckBox("addr_default", false), "VIEWBOXCAPTION");
                    }
                ...
                if (sHidden == "Save")
                {
                    ...
                    Record AddressRec = FindRecord("address_link", " adli_addressid='" + sAddressId + "'");// and adli_type is not null
                    if (!AddressRec.Eof())
                    {
                        string sBillingType = "";
                ...
                if (sHiddenDelete == "Delete")
                {
                    ...
                    string sURL = UrlDotNet(this.ThisDotNetDll, "RunPublisherConfirmDelete&pbls_PublishersID=" + sPublishrId + "&Addr_AddressId=" + sAddressId);
                    Dispatch.Redirect(sURL);
                } 
            }
        ...
        {
            Record objAddressLinkRec = FindRecord("address_link", "adli_addressid=" + sAddressId + " and adli_publisherid=" + sPublishrId + " and LTRIM(RTRIM(adli_type))='" + sType.Trim() + "' ");
            if (!objAddressLinkRec.Eof())
            {
                ...
                objAddressLinkRec.SetField("adli_publisherid", sPublishrId);
        ...
        public void CreateNewAddressLink(string AddressId, string sFrom)
        {
            ...
            objAddressLinkRec.SetField("AdLi_CompanyID", sCompanyid);
            objAddressLinkRec.SetField("AdLi_PersonID", sPersonId);
            objAddressLinkRec.SetField("adli_ClientId", sPublishrId);
        ...
        {
            Record objBuildingRec = FindRecord("publishers", "pbls_PublishersID=" + sPublishrId + " and pbls_Deleted is null");
            if (!objBuildingRec.Eof())
            {
                objBuildingRec.SetField("pbls_primarypublisherid", sAddressId);
        ...
        public void DeleteDefaultAddress()
        {
            ...
            if (!objBuildingRec.Eof())
            {
                objBuildingRec.SetField("pbls_primarypublisherid", "");
                objBuildingRec.SaveChanges();
            }
        }
